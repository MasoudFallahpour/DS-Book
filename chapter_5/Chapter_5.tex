...
\Function{Gen2Bin}{$\id{gTree}$}
		\If{$\id{gTree} \isequal \const{null}$}
				\State	\Return \const{null}
		\EndIf
		\State	\bcall{New}{$\id{bTree}$}\label{ch4:alg:ln:oneNodeGTreeBegin}
		...
		\State	$\attrib{\id{bTree}}{\id{leftChild}} \gets \Call{Gen2Bin}{q}$\label{ch4:alg:ln:Gen2BinRecCasePt1Begin}
		\State	$p \gets \attrib{\id{bTree}}{\id{leftChild}}$
		\State	$\attrib{p}{\id{rightChild}} \gets \const{null}$
		\State	$q \gets \attrib{q}{\id{next}}$\label{ch4:alg:ln:Gen2BinRecCasePt1End}
		\algstore{gen2binBrk}
\end{algorithmic}
\end{latin}
\end{algorithm}
...
شبه کد تابع ایجاد درخت عمومی از روی درخت دودویی در الگوریتم {\eqref{ch5:alg:bin2Gen}} آورده شده است.
