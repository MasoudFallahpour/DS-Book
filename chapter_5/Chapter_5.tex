...
\end{figure}
 ...
 \سوال تابعی بازگشتی بنویسید که با دریافت درخت دودوییِ معادلِ یک درخت عمومی، درخت عمومی را بازسازی کند (تنها ورودی تابع شما باید درخت دودویی باشد).
...
\سوال تابعی بازگشتی بنویسید که اشاره‌گر به ریشه‌ی درختی دودویی را به عنوان ورودی دریافت کرده و پهنای درخت را به عنوان خروجی بازگرداند (منظور از پهنای درخت بیشترین تعداد گره در یک سطح است).

\پاسخ

\سوال ساده‌ترین الگوریتم برای یافتن دومین بزرگترین عنصر در یک آرایه‌ی {$n$} عنصری نیاز به تقریباً {$2n$} مقایسه دارد. گام‌های کلی الگوریتمی را شرح دهید که دومین بزرگترین مقدار در یک آرایه را حداکثر {$n+2\lfloor \lg n \rfloor - 2$} مقایسه پیدا می‌کند (راهنمایی: فرض کنید تعداد عناصر آرایه زوج است و از درخت‌ برنده-بازنده\پانوشت{{\lr{winner-loser tree}}} استفاده کنید).
...
\سوال داده‌ساختار صف اولویت میانه\پانوشت{{\lr{Median Priority Queue}}}، یک داده‌ساختار شامل {$n$} عنصر با مقادیر متمایز است که می‌توان اعمال زیر را بر روی آن انجام داد:
\شروع{فقرات}
