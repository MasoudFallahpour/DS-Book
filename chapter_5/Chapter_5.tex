...
در ابتدا ببینیم از چه نوع درختی باید به چه نوع درختی برسیم. درخت سه تایی نشان داده شده در شکل {\eqref{ch5:fig:threeAryTree}} را در نظر بگیرید. اگر از روش لیست عمومی برای نمایش این درخت استفاده کنیم آنگاه به درخت عمومی نشان داده شده در شکل {\eqref{ch5:fig:generalTree}} می‌رسیم.

\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.33]{figs/ch5/three_ary_tree.pdf}
\caption{یک درخت سه تایی}\label{ch5:fig:threeAryTree}
\end{center}
\end{figure}
...
\caption{نمایش درخت شکل {\eqref{ch5:fig:threeAryTree}} با استفاده از لیست عمومی}\label{ch5:fig:generalTree}
\end{center}
\end{figure}

الگوریتم ایجاد درخت دودویی از روی درخت عمومی، باید اشاره‌گر به درختی مانند آنچه در شکل {\eqref{ch5:fig:generalTree}} نشان داده شده است را دریافت کند و با استفاده از روش فرزند چپ - همنیای راست درختی دودویی با ساختاری که در شکل {\eqref{ch5:fig:lcrsBinTree}} نشان داده شده است را به عنوان خروجی بازگرداند.

\begin{figure}
\begin{center}
\includegraphics[scale=0.33]{figs/ch5/left_child_right_sibling_binary_tree.pdf}
\caption{درخت دودویی حاصل از اجرای الگوریتم فرزند چپ - همنیای راست بر روی درخت شکل {\eqref{ch5:fig:threeAryTree}}}\label{ch5:fig:lcrsBinTree}
\end{center}
\end{figure}
...
\Function{Gen2Bin}{$\id{gTree}$}
		\If{$\id{gTree} \isequal \const{null}$}
				\State	\Return \const{null}
		\EndIf
		\State	\bcall{New}{$q$}\label{ch4:alg:ln:oneNodeGTreeBegin}
		...
		\If{$m \isequal \const{null}$}
				...
				\State	\Return $q$
		\EndIf\label{ch4:alg:ln:oneNodeGTreeEnd}
		\State	$\attrib{q}{\id{leftChild}} \gets \Call{Gen2Bin}{m}$\label{ch4:alg:ln:Gen2BinRecCasePt1Begin}
		\State	$p \gets \attrib{q}{\id{leftChild}}$
		\State	$\attrib{p}{\id{rightChild}} \gets \const{null}$
		\State	$m \gets \attrib{m}{\id{next}}$\label{ch4:alg:ln:Gen2BinRecCasePt1End}
		\algstore{gen2binBrk}
\end{algorithmic}
\end{latin}
\end{algorithm}
...
اگر درخت عمومی دارای یک گره باشد آنگاه یک گره برای درخت دودویی ایجاد شده و مقدار داده‌ای تنها گره درخت عمومی در فیلد داده‌ای گره‌ی تازه ایجاد شده قرار می‌گیرد و پس از تنظیم اشاره‌گرهای گره‌ی تازه ایجاد شده الگوریتم {\bcall{Gen2Bin}{}} اشاره‌گر به گره تازه ایجاد شده، که همان ریشه درخت دودویی است، را به عنوان خروجی بازمی‌گرداند (خطوط {\ref{ch4:alg:ln:oneNodeGTreeBegin}} تا {\ref{ch4:alg:ln:oneNodeGTreeEnd}}).

در صورتی که درخت عمومی دارای بیش از یک گره باشد آنگاه در مرحله اول (خطوط {\ref{ch4:alg:ln:Gen2BinRecCasePt1Begin}} تا {\ref{ch4:alg:ln:Gen2BinRecCasePt1End}}) با استفاده از یک فراخوانی بازگشتی، از روی زیردرختی که سمت چپ‌ترین فرزند گره‌ی ریشه‌ی درخت عمومی است، یک درخت دودویی به دست می‌آید و به عنوان فرزند چپ گره‌ی ریشه در درخت دودویی اصلی قرار می‌گیرد. در مرحله دوم (خطوط {\ref{ch4:alg:ln:Gen2BinRecCasePt2Begin}} تا {\ref{ch4:alg:ln:Gen2BinRecCasePt2End}}) به کمک فراخوانی بازگشتی، به ازای هر یک از فرزندان باقی مانده‌ی گره‌ی ریشه یک درخت دودویی به دست می‌آوریم و طبق روش فرزند چپ - همنیای راست در مکان مناسب از درخت دودویی اصلی درج می‌کنیم.

در انتهای اجرای الگوریتم اشاره‌گر {$q$} که به ریشه‌ی درخت دودویی ایجاد شده از روی درخت عمومی ورودی اشاره دارد به عنوان خروجی الگوریم برگشت داده می‌شود.

شکل‌ {\eqref{ch5:fig:lcrs}} روند ایجاد درخت دودویی پس از مراحل مختلف اجرای الگورریتم {\bcall{Gen2Bin}{}} بر روی درخت شکل  {\eqref{ch5:fig:threeAryTree}} را نشان می‌دهد.

\begin{figure}
\begin{center}
\subfloat[%
درخت دودویی پس از اجرای خطوط {\ref{ch4:alg:ln:oneNodeGTreeBegin}} تا {\ref{ch4:alg:ln:oneNodeGTreeEnd}}
]{\includegraphics[scale=0.33]{figs/ch5/lcrs_1.pdf}\label{ch5:fig:lcrs1}}\qquad
\subfloat[%
درخت دودویی پس از اجرای خطوط {\ref{ch4:alg:ln:Gen2BinRecCasePt1Begin}} تا {\ref{ch4:alg:ln:Gen2BinRecCasePt1End}}
]{\includegraphics[scale=0.33]{figs/ch5/lcrs_2.pdf}\label{ch5:fig:lcrs2}}\\[0.1cm]
\subfloat[%
درخت دودویی پس از اجرای دور اول حلقه‌
]{\includegraphics[scale=0.33]{figs/ch5/lcrs_3.pdf}\label{ch5:fig:lcrs3}}\qquad
\subfloat[%
درخت دودویی نهایی پس از خاتمه‌ی دور دوم و آخر حلقه
]{\includegraphics[scale=0.33]{figs/ch5/lcrs_4.pdf}\label{ch5:fig:lcrs4}}
\caption{%
روند ایجاد درخت دودویی با اجرای الگوریتم {\lr{\textsc{Gen2Bin}}} بر روی درخت عمومی شکل {\eqref{ch5:fig:threeAryTree}}
}\label{ch5:fig:lcrs}
\end{center}
\end{figure}
